...
class ThreadRepositoryPostgres extends ThreadRepository {
  ...
  async getThreadById(threadId) {
    const query = {
      text: `SELECT t.id, t.title, t.body, (t.date AT TIME ZONE 'Asia/Jakarta') AS date, u.username
  ...
  async getAllThreads() {
    const query = {
      text: `SELECT t.id, t.title, (t.date AT TIME ZONE 'Asia/Jakarta') AS date, u.username
             FROM threads t JOIN users u ON u.id = t.owner`,
    };
    const result = await this._pool.query(query);
    return result.rows;
